...
`origin/documentation` points to `7f8dce5` and adds these documents:
...
successfully by run `29247907376`.
...
`Getting Started` is a top-level Starlight sidebar item, outside the collapsible
Guides group, so it remains visible while browsing other documentation pages.
