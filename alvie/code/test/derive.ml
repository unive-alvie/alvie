  ...
  (* let sul =
    Sancus.Verilog.make
      ...
      let ol = List.t_of_sexp Output_internal.t_of_sexp (Sexp.of_string out_sexp) in
        let st = Time_ns.now () in
          let i = match Inputgen.generate_next spec_dfa il ol with `Next i -> i | _ -> failwith "Should never happen!" in
          let et = Time_ns.now () in
          Logs.debug (fun p -> p "Inputgen.generate_next: %s" (Time_ns.Span.to_string (Time_ns.diff et st)));
        let st = Time_ns.now () in
          ignore (Inputgen.matchable spec_dfa i il ol);
          let et = Time_ns.now () in
          Logs.debug (fun p -> p "Inputgen.matchable: %s" (Time_ns.Span.to_string (Time_ns.diff et st)))
