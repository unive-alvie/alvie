#!/bin/bash
# Once-per-session setup for the FPGA pmem build pipeline.
# Extracts memory sizes and generates linker script + defs file so
# build_pmem_fpga only needs to run msp430-as + msp430-ld per step.
#
# Usage: setup_fpga.sh <tmpdir>
...
tmpdir="$1"
# sancus_dir: path to the sancus-core-gap directory.
# Defaults to $tmpdir/sancus-core-gap for the simulator path (which uses git archive).
# The FPGA path passes the live checkout directly to skip git archive.
sancus_dir="${2:-$tmpdir/sancus-core-gap}"
scriptsdir="$(dirname "$0")"
srcdir="$scriptsdir/../src"

incfilev="$sancus_dir/core/rtl/verilog/openMSP430_defines.v"
incfile="$tmpdir/pmem.h"
linkfile="$sancus_dir/core/sim/rtl_sim/bin/template.x"
headfile="$sancus_dir/core/sim/rtl_sim/bin/template_defs.asm"
